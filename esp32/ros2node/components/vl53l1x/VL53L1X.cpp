...
{
    uint8_t buf[256];
    read(0x00C0, sizeof(buf), buf, pdMS_TO_TICKS(I2C_TIMEOUT));
...
void VL53L1X::writeReg(uint16_t reg, uint8_t value)
{
    ...
    if((CommandHandle = i2c_cmd_link_create()) != NULL) {
        ...
        ESP_ERROR_CHECK_NONFATAL(
            i2c_master_write_byte(CommandHandle, (address << 1) | I2C_MASTER_WRITE, true), last_status = 2; return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((reg >> 8) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((reg >> 0) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 0) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_stop(CommandHandle), last_status = 4; return );
        if(ESP_OK == i2c_master_cmd_begin((i2c_port_t)I2CPortNumber, CommandHandle, pdMS_TO_TICKS(I2C_TIMEOUT))) {
            ESP_LOGI(TAG, "writeReg16Bit(%04x,%02x) ... ok", reg, value);
        } else {
            last_status = 3;
        }
...
void VL53L1X::writeReg16Bit(uint16_t reg, uint16_t value)
{
    ...
    if((CommandHandle = i2c_cmd_link_create()) != NULL) {
        ...
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((reg >> 0) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 8) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 0) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_stop(CommandHandle), last_status = 4; return );
        if(ESP_OK == i2c_master_cmd_begin((i2c_port_t)I2CPortNumber, CommandHandle, pdMS_TO_TICKS(I2C_TIMEOUT))) {
            ESP_LOGI(TAG, "writeReg16Bit(%04x,%04x) ... ok", reg, value);
        } else {
            last_status = 3;
        }
        i2c_cmd_link_delete(CommandHandle);
...
void VL53L1X::writeReg32Bit(uint16_t reg, uint32_t value)
{
    ...
    if((CommandHandle = i2c_cmd_link_create()) != NULL) {
        ...
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((reg >> 0) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 24) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 16) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 8) & 0xFF), true), last_status = 3;
                                 return );
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((value >> 0) & 0xFF), true), last_status = 3;
        ...
        ESP_ERROR_CHECK_NONFATAL(i2c_master_stop(CommandHandle), last_status = 4; return );
        if(ESP_OK == i2c_master_cmd_begin((i2c_port_t)I2CPortNumber, CommandHandle, pdMS_TO_TICKS(I2C_TIMEOUT))) {
            ESP_LOGI(TAG, "writeReg32Bit(%04x,%08x) ... ok", reg, value);
        } else {
            last_status = 3;
        }
        i2c_cmd_link_delete(CommandHandle);
...
int VL53L1X::read(uint16_t reg, int n, uint8_t* buf, TickType_t ticks_to_wait)
{
    ...
    if((CommandHandle = i2c_cmd_link_create()) != NULL) {
        ...
        i2c_master_write_byte(CommandHandle, ((reg >> 8) & 0xFF), true);
        i2c_master_write_byte(CommandHandle, ((reg >> 0) & 0xFF), true);
        i2c_master_stop(CommandHandle);
        err = i2c_master_cmd_begin((i2c_port_t)I2CPortNumber, CommandHandle, ticks_to_wait);
        if(err != ESP_OK) {
            ESP_LOGE(TAG, "VL53L1X::read i2caddr=0x%02x regaddr=0x%02x err=0x%02x", address, reg, err);
            retval = -1;
        } else {
            if((CommandHandle = i2c_cmd_link_create()) != NULL) {
                i2c_master_start(CommandHandle);
                i2c_master_write_byte(CommandHandle, (address << 1) | I2C_MASTER_READ, true);
                i2c_master_read(CommandHandle, buf, n, I2C_MASTER_LAST_NACK);
                i2c_master_stop(CommandHandle);
                err = i2c_master_cmd_begin((i2c_port_t)I2CPortNumber, CommandHandle, ticks_to_wait);
                if(err != ESP_OK) {
                    ESP_LOGE(TAG, "VL53L1X::read i2caddr=0x%02x regaddr=0x%02x err=0x%02x", address, reg, err);
    ...
    if((CommandHandle = i2c_cmd_link_create()) != NULL) {
        ...
        ESP_ERROR_CHECK_NONFATAL(
            i2c_master_write_byte(CommandHandle, (address << 1) | I2C_MASTER_WRITE, true), last_status = 2;
            return retval);
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((reg >> 8) & 0xFF), true), last_status = 3;
                                 return retval);
        ESP_ERROR_CHECK_NONFATAL(i2c_master_write_byte(CommandHandle, ((reg >> 0) & 0xFF), true), last_status = 3;
        ...
        ESP_ERROR_CHECK_NONFATAL(
            i2c_master_write_byte(CommandHandle, (address << 1) | I2C_MASTER_READ, true), last_status = 2;
            return retval);
        if(n > 1) {
            ESP_ERROR_CHECK_NONFATAL(i2c_master_read(CommandHandle, &buf[0], n - 1, I2C_MASTER_ACK), last_status = 3;
                                     return retval);
        }
        ESP_ERROR_CHECK_NONFATAL(i2c_master_read(CommandHandle, &buf[n - 1], 1, I2C_MASTER_NACK), last_status = 3;
                                 return retval);
...
{
    uint8_t buf[1];
    if(read(reg, sizeof(buf), buf, pdMS_TO_TICKS(I2C_TIMEOUT)) == sizeof(buf)) {
...
uint32_t VL53L1X::readReg32Bit(uint16_t reg)
{
    uint8_t buf[4];
    if(read(reg, sizeof(buf), buf, pdMS_TO_TICKS(I2C_TIMEOUT)) == sizeof(buf)) {
        ESP_LOGI(TAG, "readReg32Bit(reg=%02x) => %04x", reg,
...
uint32_t VL53L1X::getMeasurementTimingBudget()
{
    ...
    uint32_t range_config_timeout_us =
        timeoutMclksToMicroseconds(decodeTimeout(readReg16Bit(RANGE_CONFIG__TIMEOUT_MACROP_A)), macro_period_us);
...
void VL53L1X::readResults()
{
    ...
    uint8_t tmp[19];
    if(read(RESULT__RANGE_STATUS, sizeof(tmp), tmp, pdMS_TO_TICKS(I2C_TIMEOUT)) == sizeof(tmp)) {
        int j = 0;

        ESP_LOGI(TAG,
            "readResults() %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x ... done",
        ...
        ESP_LOGE(TAG, "readResults() ... error");
    }
}
