...
extern uint8_t s_ip_addr_changed;
...
{
    http_buffer_t fb_data = { .data = camera_get_fb(), .size = camera_get_data_size(), .data_is_persistent = true };
...
static void handle_rgb_bmp_stream(http_context_t http_ctx, void* ctx)
{
	...
	err = http_response_begin_multipart(http_ctx, "image/bitmap", camera_get_data_size() + sizeof(*header));
	if(err != ESP_OK) {
	    break;
	}
...
static void handle_jpg_stream(http_context_t http_ctx, void* ctx)
{
	...
	err = http_response_begin_multipart(http_ctx, "image/jpg", camera_get_data_size());
	if(err != ESP_OK) {
	    break;
	}
	err = write_frame(http_ctx);
