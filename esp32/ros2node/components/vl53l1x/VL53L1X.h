...
class VL53L1X
{
  public:
   ...
   // RangeInvalid              =  14: can't actually be returned by API (range can never become negative, even after correction)
    ...
    int read(uint16_t reg, int n, uint8_t *buf,TickType_t ticks_to_wait);
  ...
  private:
   ...
   // uint16_t phase_sd0: not used
    ...
    void startTimeout() { timeout_start_ms = esp_timer_get_time()/1000; }

    // Check if timeout is enabled (set to nonzero value) and has expired
    bool checkTimeoutExpired() {return (io_timeout > 0) && ((uint16_t)((esp_timer_get_time()/1000) - timeout_start_ms) > io_timeout); }
